"""
gen-year-audio-to-r2.py
使用 Azure TTS 生成年份听写音频，并直接上传到 Cloudflare R2。

示例：
  .venv/bin/python gen-year-audio-to-r2.py
  .venv/bin/python gen-year-audio-to-r2.py --dry-run

断点续跑：
  已上传成功的 R2 key 会记录在 upload-to-r2/r2_year_success.log。
  已生成的本地 mp3 会保留在 local-audios/year，重跑时不会重复合成。
"""
...
LOCAL_AUDIO_DIR = BASE_DIR / "local-audios" / "year"
LOG_DIR = BASE_DIR / "upload-to-r2"
SUCCESS_LOG = LOG_DIR / "r2_year_success.log"
FAILED_LOG = LOG_DIR / "r2_year_failed.log"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="生成年份听写音频并上传到 R2。")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="只展示将处理的年份，不生成或上传。",
    )
    parser.add_argument(
        "--no-skip",
        action="store_true",
...
def build_audio_filename(year: str) -> str:
    return year.replace("/", "-") + ".mp3"
...
def synthesize_to_file(spoken_text: str, local_path: Path) -> tuple[bool, str]:
  ...
  <voice name="{VOICE_NAME}">
    ...
    headers = {
        ...
        "User-Agent": "radom-sound-year-audio",
    }
...
def main() -> None:
    ...
    pending = []
    for year, spoken_text in YEAR_WORDS:
        filename = build_audio_filename(year)
        if filename in done_success:
            continue
        pending.append((year, spoken_text, filename))

    log.info("年份音频共 %d 条，已完成 %d 条，本次处理 %d 条", len(YEAR_WORDS), len(YEAR_WORDS) - len(pending), len(pending))
    log.info("R2 bucket: %s", R2_BUCKET_NAME)
    log.info("R2 prefix: (根目录)")

    if args.dry_run:
        for year, spoken_text, filename in pending:
            log.info("[dry-run] %s (%s) -> %s", year, spoken_text, filename)
        return

    r2 = build_r2_client()
    success_count = failed_count = 0

    for idx, (year, spoken_text, filename) in enumerate(pending, 1):
        local_path = LOCAL_AUDIO_DIR / filename
        log.info("[%d/%d] %s (%s) -> %s", idx, len(pending), year, spoken_text, filename)
        ...
        if not local_path.exists():
            ...
            if not synth_ok:
                if local_path.exists() and local_path.stat().st_size == 0:
                    local_path.unlink()
                log.error("  合成彻底失败: %s", year)
                _append_log(FAILED_LOG, f"synthesize\t{year}")
                failed_count += 1
                continue
        else:
